...
register_google(key = "AIzaSyAcShaycW9cth1M8p7FbZ03C-nkY8la7WU",
...
#ggsave(filename="Figures/estuary_diversity.png", 
#       plot = g1, width = 15, height = 15, dpi = 300, units = "in")
...
p5a <- ggiNEXT(pred.re.300, type=1) + 
        ...
        labs(title="A Sample size-based R/E curve") +
        theme(panel.background=element_rect(fill="white"),
              panel.grid.major=element_blank(), 
              panel.grid.minor=element_blank(),
              axis.line.x=element_line(colour="black", size=0.5, linetype="solid"),
...
p5b <- ggiNEXT(pred.re.300, type=2) +
        ...
        labs(title="B Sample completeness curve") +
        theme(panel.background=element_rect(fill="white"),
              panel.grid.major=element_blank(), 
              panel.grid.minor=element_blank(),
              axis.line.x=element_line(colour="black", size=0.5, linetype="solid"),
...
p5c <- ggiNEXT(pred.re.300, type=3) + 
        ...
        scale_y_continuous(limits=c(0,50), breaks=seq(0,50,10)) +
        labs(title="C Coverage-based R/E curve") +
        theme(panel.background=element_rect(fill="white"),
              panel.grid.major=element_blank(), 
              panel.grid.minor=element_blank(),
              axis.line.x=element_line(colour="black", size=0.5, linetype="solid"),
...
## Summarise plots
g3 <- ggarrange(p5a, p5b, p5c, ncol=1, nrow=3,
                common.legend=TRUE, legend="bottom")

#ggsave(filename="Figures/FigS7.png", 
#       plot = g3, width = 10, height = 13, dpi = 300, units = "in")
...
p7a <- ggplot(pred.turn.NMDS, aes(x=NMDS1, y=NMDS2, colour=Predator)) + 
        ...
        scale_x_continuous(limits=c(-0.6,1), breaks=seq(-0.6,1,0.2),
                           labels=scales::number_format(accuracy = 0.1)) + 
        scale_y_continuous(limits=c(-0.03,0.035), breaks=seq(-0.03,0.03,0.015),
                                labels=scales::number_format(accuracy = 0.001)) + 
        labs(title=expression(bold("B "~beta~"Diversity")), 
             subtitle="(i) Turnover", 
             x="NMDS1",y="NMDS2") + 
...
p7b <- ggplot(pred.nest.NMDS, aes(x=NMDS1, y=NMDS2, colour=Predator)) + 
        ...
        labs(title="", 
             ...
             x="NMDS1", y="NMDS2") + 
        annotate("text", x=0.45, y=0.6, label="stress = 0.193", cex=5) + 
...
p7c <- ggplot(pred.total.NMDS, aes(x=NMDS1, y=NMDS2, colour=Predator)) + 
        ...
        labs(title="",
             ...
             x="NMDS1", y="NMDS2") + 
        annotate("text", x=0.85, y=0.04, label="stress = 0.043", cex=5) + 
        scale_colour_manual(name="Predator",
                            values=c("limegreen","purple")) + 
        guides(colour=guide_legend(nrow=2, byrow=TRUE)) + 
        theme(panel.background = element_rect(fill = 'white'),
...
## with each site.
MT <- droplevels(subset(pred.metadata, select="Sequencing_ID", Site == "Malham Tarn"))
RG <- droplevels(subset(pred.metadata, select="Sequencing_ID", Site == "River Glaven"))
RH <- droplevels(subset(pred.metadata, select="Sequencing_ID", Site == "River Hull"))
...
RG.richness <- append(RG.richness, 37, after=0)
RH.richness <- append(RH.richness, 125, after=0)
...
## Summarise plots
g6 <- ggarrange(p9a, p9b, p9c, ncol=1, nrow=3,
                common.legend=TRUE, legend="bottom")

#ggsave(filename="Figures/FigS8.png", 
#       plot = g6, width = 10, height = 13, dpi = 300, units = "in")
...
p10a <- ggplot(site.turn.NMDS, aes(x=NMDS1, y=NMDS2, colour=Site)) + 
        ...
        scale_x_continuous(limits=c(-0.2,1), breaks=seq(-0.2,1,0.2),
                           labels=scales::number_format(accuracy = 0.1)) + 
        scale_y_continuous(limits=c(-0.03,0.01), breaks=seq(-0.03,0.01,0.01),
                           labels=scales::number_format(accuracy = 0.01)) + 
        labs(title=expression(bold("B "~beta~"Diversity")),
             subtitle="(i) Turnover", 
             x="NMDS1",y="NMDS2") + 
        annotate("text", x=0.85, y=0.01, label="stress = 0.077", cex=5) + 
...
p10c <- ggplot(site.total.NMDS, aes(x=NMDS1, y=NMDS2, colour=Site)) + 
        ...
        scale_x_continuous(limits=c(-0.2,1), breaks=seq(-0.2,1,0.2),
                           labels=scales::number_format(accuracy = 0.1)) + 
        scale_y_continuous(limits=c(-0.02,0.02), breaks=seq(-0.02,0.02,0.01), 
                           labels=scales::number_format(accuracy = 0.01)) + 
        labs(title="", 
             subtitle=expression(bold("(iii) Total"~beta~"Diversity")),
             x="NMDS1",y="NMDS2") + 
...
#       plot = g6, width = 13, height = 13, dpi = 300, units = "in")
