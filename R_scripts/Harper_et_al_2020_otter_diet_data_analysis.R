#' ---
#' Title: "DNA metabarcoding shows strong potential for investigating diet and niche partitioning in the native European otter (Lutra lutra) and invasive American mink (Neovison vison)"
...
p <- c("ggplot2","ggpubr","scales","ggmap","legendMap","gtools","dplyr",
       "plyr","tidyr","forcats","reshape2","bipartite","car","multcomp",
       "FSA","lawstat","vegan","betapart")
...
register_google(key = "AIzaSyB1NGlXTsJxJofpM_Iq7vUQosIRjpZAy8A", 
...
m1 <- m1 + geom_point(data=metadata, 
                      aes(x=Longitude, y=Latitude, shape=Read_ID), 
                      size=2)
m1 <- m1 + scale_shape_manual(name="Predator",
                              values=c(15,4,16,8,17,18))
m1 <- m1 + scale_bar(lon=-3.5, lat=52, 
                     distance_lon=50, distance_lat=8, 
                     distance_legend=20, dist_unit="km",
                     arrow_length=50, arrow_distance=30, 
                     arrow_north_size=5)
m1 <- m1 + labs(title="A  UK", x="Longitude", y="Latitude")
m1 <- m1 + theme_bw()
m1 <- m1 + theme(plot.title = element_text(face="bold"),
...
m2 <- m2 + geom_point(data=Malham.coords, 
                      aes(x=Longitude, y=Latitude), 
                      size=2, pch=17)
m2 <- m2 + scale_bar(lon=-2.183, lat=54.083, 
                     distance_lon=0.5, distance_lat=0.08, 
                     distance_legend=0.15, dist_unit="km",
                     arrow_length=0.5, arrow_distance=0.2, 
                     arrow_north_size=5)
m2 <- m2 + labs(title="B  Malham Tarn", x="Longitude", y="Latitude")
m2 <- m2 + theme_bw()
m2 <- m2 + theme(plot.title = element_text(face="bold"),
                 axis.title.x = element_text(margin = unit(c(5, 0, 0, 0), "mm")),
                 axis.title.y = element_text(margin = unit(c(0, 5, 0, 0), "mm")),
                 axis.text.x = element_text(colour="black"),
                 axis.text.y = element_text(colour="black"),
...
m3 <- m3 + geom_point(data=Hull.coords, 
                      aes(x=Longitude, y=Latitude, shape=Read_ID), 
                      size=2)
m3 <- m3 + scale_shape_manual(name="Predator",
                              values=c(15,4,16,8,17,18))
m3 <- m3 + scale_bar(lon=-0.57, lat=53.81, 
                     distance_lon=5, distance_lat=0.6, 
                     distance_legend=1.2, dist_unit="km",
                     arrow_length=3, arrow_distance=2, 
                     arrow_north_size=5)
m3 <- m3 + labs(title="C  River Hull", x="Longitude", y="Latitude")
m3 <- m3 + theme_bw()
m3 <- m3 + theme(plot.title = element_text(face="bold"),
                 axis.title.x = element_text(margin = unit(c(5, 0, 0, 0), "mm")),
                 axis.title.y = element_text(margin = unit(c(0, 5, 0, 0), "mm")),
                 axis.text.x = element_text(colour="black"),
                 axis.text.y = element_text(colour="black"),
...
m4 <- m4 + geom_point(data=Glaven.coords, 
                      aes(x=Longitude, y=Latitude, shape=Read_ID), 
                      size=2)
m4 <- m4 + scale_shape_manual(name="Predator",
                              values=c(15,4,16,8,17,18))
m4 <- m4 + scale_bar(lon=0.995, lat=52.847, 
                     distance_lon=2, distance_lat=0.3, 
                     distance_legend=0.6, dist_unit="km",
                     arrow_length=1.5, arrow_distance=1, 
                     arrow_north_size=5)
m4 <- m4 + labs(title="D  River Glaven", x="Longitude", y="Latitude")
m4 <- m4 + theme_bw()
m4 <- m4 + theme(plot.title = element_text(face="bold"),
                 axis.title.x = element_text(margin = unit(c(5, 0, 0, 0), "mm")),
                 axis.title.y = element_text(margin = unit(c(0, 5, 0, 0), "mm")),
                 axis.text.x = element_text(colour="black"),
                 axis.text.y = element_text(colour="black"),
...
hm0 <- ggplot(contaminants, aes(x=ID, 
                                ...
                                fill=PRC))
hm0 <- hm0 + geom_tile(colour="black")
hm0 <- hm0 + scale_fill_gradient(name="Proportional\nread counts", 
                                 limits=c(0,1),
                                 breaks=c(0,0.25,0.50,0.75,1),
                                 low="white", high="red",
                                 guide=guide_colourbar(frame.colour="black",
                                                       ticks.colour="black"))
hm0 <- hm0 + labs(x="PCR controls", y="Taxonomic assignment")
hm0 <- hm0 + theme_bw()
...
hm0 <- hm0 + facet_grid(. ~ Type, scales="free",space="free")
hm0
...
hm1 <- ggplot(compare.threshold, aes(x=ID, 
                                     y=fct_rev(as_factor(Assignment)), 
                                     fill=PRC))
hm1 <- hm1 + geom_tile(colour="black")
hm1 <- hm1 + scale_fill_gradient(name="Proportional\nread counts    ", 
                                 ...
                                 guide=guide_colourbar(frame.colour="black",
                                                       ticks.colour="black"))
hm1 <- hm1 + scale_y_discrete(labels=rev(labels))
hm1 <- hm1 + labs(x="Samples", y="Taxonomic assignment")
hm1 <- hm1 + theme_bw()
...
hm1 <- hm1 + facet_grid(. ~ fThreshold, scales="free", space="free")
hm1
...
ggarrange(pie1, pie2, pie3, pie4, ncol=2, nrow=2,
          common.legend=TRUE, legend="bottom")
...
hm3 <- ggplot(prc.plot, aes(x=Sample, 
                            ...
                            fill=PRC))
hm3 <- hm3 + geom_tile(aes(colour=Group), size=0.5)
hm3 <- hm3 + scale_colour_manual(values=c("limegreen","goldenrod1",
                                          "dodgerblue","hotpink"))
...
hm3 <- hm3 + theme(panel.grid.major = element_line(colour="white"),
                   ...
                   text = element_text(size=20),
                   legend.key.size = unit(1, 'lines'))
hm3 <- hm3 + facet_grid(Site ~ Predator, scales="free", space="free")
hm3
...
p1 <- p1 + scale_y_continuous(limits=c(0,100),
                              breaks=seq(0,100,10))
p1 <- p1 + scale_fill_manual(name="Group",
                             values=c("limegreen","goldenrod1",
                                      "dodgerblue","hotpink"))
p1 <- p1 + labs(x="Taxonomic assignment", y="Samples (%)")
p1 <- p1 + geom_text(aes(label=Count, y=Proportion), 
...
p1 <- p1 + theme(panel.grid.major = element_line(colour="white"),
                 ...
                 legend.position="bottom")
p1 <- p1 + facet_grid(Predator ~ ., 
                      labeller=labeller(Predator=predator.names))
p1
...
p2 <- p2 + geom_bar(stat="identity", colour="black")
p2 <- p2 + scale_y_continuous(limits=c(0,100),
                              breaks=seq(0,100,10))
p2 <- p2 + scale_fill_manual(guide=FALSE,
                             breaks=c("Bufo bufo","Pelophylax ridibundus",
                                      "Rana temporaria","Barbatula barbatula",
                                      "Cottus gobio","Gasterosteus aculeatus",
                                      "Gobio gobio","Phoxinus phoxinus",
                                      "Pungitius pungitius","Carassius carassius", 
...
p2 <- p2 + geom_text(aes(label=Count, y=Proportion), stat="identity", vjust = -.5)
p2 <- p2 + theme_bw()
p2 <- p2 + theme(panel.grid.major = element_line(colour="white"),
                 panel.grid.minor = element_line(colour="white"),
                 axis.title.x = element_text(margin = unit(c(5, 0, 0, 0), "mm")),
                 axis.title.y = element_text(margin = unit(c(0, 5, 0, 0), "mm")),
                 axis.text.x = element_text(colour="black", angle = 60, vjust=1, hjust=1),
                 ...
                 text = element_text(size=16),
                 legend.position="bottom")
p2 <- p2 + facet_grid(Predator ~ Site, scales="free", space="free")
p2
...
#seq.low <- c(amphibian, fish, bird, mammal)
...
p3 <- p3 + geom_jitter(aes(colour=Predator), 
                       cex=2, 
                       width=0.2, 
                       show.legend=FALSE)
p3 <- p3 + geom_boxplot(alpha=0.7,
                        outlier.shape=NA)
p3 <- p3 + scale_y_continuous(limits=c(0,10),
                              breaks=seq(0,10,2))
p3 <- p3 + labs(x="Predator", y="Taxon richness")
p3 <- p3 + scale_colour_manual(values=c("limegreen","purple"))
p3 <- p3 + theme_bw()
p3 <- p3 + theme(panel.background = element_rect(fill = 'white'),
                 ...
                 legend.position = "none",
                 text = element_text(size=20))
p3 <- p3 + facet_grid(Waterbody ~ Site, scales="free", space="free")
p3
...
p4 <- p4 + geom_jitter(cex=2, 
                       width=0.15,
                       height=0.2)
p4 <- p4 + geom_boxplot(colour="black",
                        alpha=0.7,
                        outlier.shape=NA)
p4 <- p4 + scale_y_continuous(limits=c(0,10),
                              breaks=seq(0,10,1))
p4 <- p4 + annotate("text", x = c("Mink","Otter"), y = 10, 
                    label = c("a","b"), cex=10)
p4 <- p4 + labs(title=expression(bold("A"~alpha~"Diversity")),
                x="Predator", y="Taxon richness")
p4 <- p4 + scale_colour_manual(values=c("limegreen","purple"))
...
p5 <- ggplot(otter.alpha, aes(x=Site, y=sample.richness, colour=Site))
p5 <- p5 + geom_jitter(cex=2, 
                       width=0.15,
                       height=0.2)
p5 <- p5 + geom_boxplot(colour="black",
                        alpha=0.7,
                        outlier.shape=NA)
p5 <- p5 + annotate("text", x = c("Malham Tarn",
                                  "River Glaven",
                                  "River Hull"), y = 10, 
                    label = c("a","a","b"), cex=10)
p5 <- p5 + scale_colour_manual(values=c("grey30","goldenrod2","dodgerblue3"))
p5 <- p5 + scale_y_continuous(limits=c(0,10),
                              breaks=seq(0,10,1))
p5 <- p5 + labs(title=expression(bold("A"~alpha~"Diversity")), 
...
pred.metadata <- diet.metadata[diet.metadata$Sequencing_ID %in% rownames(pred.beta.jac),]
...
p6a <- p6a + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p6a <- p6a + scale_x_continuous(limits=c(-0.5,0.8), breaks=seq(-0.5,0.8,0.1),
                                labels=scales::number_format(accuracy = 0.1))
p6a <- p6a + scale_y_continuous(limits=c(-0.1,0.4), breaks=seq(-0.1,0.4,0.1),
                                labels=scales::number_format(accuracy = 0.1))
p6a <- p6a + labs(title="A Jaccard dissimilarity", 
                  subtitle="(i) All data", 
                  x="NMDS1",y="NMDS2")
p6a <- p6a + annotate("text", x=0.65, y=0.4, label="stress = 0.0002", cex=5)
p6a <- p6a + scale_colour_manual(name="Predator",
...
p6b <- p6b + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p6b <- p6b + scale_x_continuous(limits=c(-0.1,1), breaks=seq(-0.1,1,0.1),
                                labels=scales::number_format(accuracy = 0.1))
p6b <- p6b + scale_y_continuous(limits=c(-0.3,0.3), breaks=seq(-0.3,0.3,0.15),
                                labels=scales::number_format(accuracy = 0.01))
p6b <- p6b + labs(title="B Bray-Curtis dissimilarity", 
                  subtitle="(i) All data", 
                  x="NMDS1",y="NMDS2")
p6b <- p6b + annotate("text", x=0.8, y=0.3, label="stress = 0.0003", cex=5)
p6b <- p6b + scale_colour_manual(name="Predator",
                                 values=c("limegreen","purple"))
...
p6c <- p6c + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p6c <- p6c + scale_x_continuous(limits=c(-0.6,1), breaks=seq(-0.6,1,0.2),
                                labels=scales::number_format(accuracy = 0.1))
p6c <- p6c + scale_y_continuous(limits=c(-0.03,0.02), breaks=seq(-0.03,0.02,0.01),
                                labels=scales::number_format(accuracy = 0.01))
p6c <- p6c + labs(title="", 
                  subtitle="(ii) Outliers removed", 
                  x="NMDS1",y="NMDS2")
p6c <- p6c + annotate("text", x=0.75, y=0.02, label="stress = 0.043", cex=5)
p6c <- p6c + scale_colour_manual(name="Predator",
                                 values=c("limegreen","purple"))
...
p6d <- p6d + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p6d <- p6d + scale_x_continuous(limits=c(-0.6,1), breaks=seq(-0.6,1,0.2),
                                labels=scales::number_format(accuracy = 0.1))
p6d <- p6d + scale_y_continuous(limits=c(-0.02,0.02), breaks=seq(-0.02,0.02,0.01),
                                labels=scales::number_format(accuracy = 0.01))
p6d <- p6d + labs(title="", 
                  subtitle="(ii) Outliers removed", 
                  x="NMDS1",y="NMDS2")
p6d <- p6d + annotate("text", x=0.75, y=0.02, label="stress = 0.043", cex=5)
p6d <- p6d + scale_colour_manual(name="Predator",
                                 values=c("limegreen","purple"))
p6d <- p6d + theme(panel.background = element_rect(fill = 'white'),
                   axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),
                   axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid'),
                   axis.title.x = element_text(margin = unit(c(8, 0, 0, 0), "mm")),
                   axis.title.y = element_text(margin = unit(c(0, 5, 0, 0), "mm")),
...
p7a <- p7a + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p7a <- p7a + scale_x_continuous(limits=c(-0.6,1), breaks=seq(-0.6,1,0.2),
                                labels=scales::number_format(accuracy = 0.1))
p7a <- p7a + scale_y_continuous(limits=c(-0.03,0.035), breaks=seq(-0.03,0.03,0.015),
                                labels=scales::number_format(accuracy = 0.001))
p7a <- p7a + labs(title=expression(bold("B "~beta~"Diversity")), 
                  subtitle="(i) Turnover", 
                  x="NMDS1",y="NMDS2")
p7a <- p7a + annotate("text", x=0.85, y=0.03, label="stress = 0.043", cex=5)
p7a <- p7a + scale_colour_manual(name="Predator",
                                 values=c("limegreen","purple"))
p7a <- p7a + theme(panel.background = element_rect(fill = 'white'),
                   axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),
                   axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid'),
                   axis.title.x = element_text(margin = unit(c(8, 0, 0, 0), "mm")),
                   axis.title.y = element_text(margin = unit(c(0, 5, 0, 0), "mm")),
...
p7b <- p7b + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p7b <- p7b + scale_x_continuous(limits=c(-0.6,0.6), breaks=seq(-0.6,0.6,0.2),
                                labels=scales::number_format(accuracy = 0.1))
p7b <- p7b + scale_y_continuous(limits=c(-0.6,0.65), breaks=seq(-0.6,0.6,0.3),
                                labels=scales::number_format(accuracy = 0.1))
p7b <- p7b + labs(title="", 
                  subtitle="(ii) Nestedness",
                  x="NMDS1", y="NMDS2")
p7b <- p7b + annotate("text", x=0.45, y=0.6, label="stress = 0.188", cex=5)
p7b <- p7b + scale_colour_manual(name="diet set",
                                 values=c("limegreen","purple"))
p7b <- p7b + guides(colour=guide_legend(nrow=2, byrow=TRUE))
p7b <- p7b + theme(panel.background = element_rect(fill = 'white'),
                   axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),
                   axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid'),
                   axis.title.x = element_text(margin = unit(c(8, 0, 0, 0), "mm")),
                   axis.title.y = element_text(margin = unit(c(0, 5, 0, 0), "mm")),
...
p7c <- p7c + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p7c <- p7c + scale_x_continuous(limits=c(-0.6,1), breaks=seq(-0.6,1,0.2),
                                labels=scales::number_format(accuracy = 0.1))
p7c <- p7c + scale_y_continuous(limits=c(-0.025,0.04), breaks=seq(-0.02,0.04,0.02),
                                labels=scales::number_format(accuracy = 0.001))
p7c <- p7c + labs(title="",
                  subtitle=expression(bold("(iii) Total"~beta~"Diversity")),
                  x="NMDS1", y="NMDS2")
p7c <- p7c + annotate("text", x=0.85, y=0.04, label="stress = 0.043", cex=5)
p7c <- p7c + scale_colour_manual(name="Predator",
                                 values=c("limegreen","purple"))
p7c <- p7c + guides(colour=guide_legend(nrow=2, byrow=TRUE))
p7c <- p7c + theme(panel.background = element_rect(fill = 'white'),
                   axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),
                   axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid'),
                   axis.title.x = element_text(margin = unit(c(8, 0, 0, 0), "mm")),
                   axis.title.y = element_text(margin = unit(c(0, 5, 0, 0), "mm")),
...
## Create vectors of sample IDs associated with otter
otter <- droplevels(subset(diet.metadata, select="Sequencing_ID", Predator == "Otter"))
...
## Create vectors of sample IDs associated with each site
MT <- droplevels(subset(otter.metadata, select="Sequencing_ID", Site == "Malham Tarn"))
RG <- droplevels(subset(otter.metadata, select="Sequencing_ID", Site == "River Glaven"))
RH <- droplevels(subset(otter.metadata, select="Sequencing_ID", Site == "River Hull"))
...
## Plot data frame
p8a <- ggplot(site.turn.NMDS, aes(x=NMDS1, y=NMDS2, colour=Site))
p8a <- p8a + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p8a <- p8a + scale_x_continuous(limits=c(-0.2,1), breaks=seq(-0.2,1,0.2),
                                labels=scales::number_format(accuracy = 0.1))
p8a <- p8a + scale_y_continuous(limits=c(-0.03,0.01), breaks=seq(-0.03,0.01,0.01),
                                labels=scales::number_format(accuracy = 0.01))
p8a <- p8a + labs(title=expression(bold("B "~beta~"Diversity")),
                  subtitle="(i) Turnover", 
                  x="NMDS1",y="NMDS2")
p8a <- p8a + annotate("text", x=0.85, y=0.01, label="stress = 0.077", cex=5)
p8a <- p8a + scale_colour_manual(name="Site",
...
## Plot data frame
p8b <- ggplot(site.nest.NMDS, aes(x=NMDS1, y=NMDS2, colour=Site))
p8b <- p8b + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p8b <- p8b + scale_x_continuous(limits=c(-0.5,0.5), breaks=seq(-0.5,0.5,0.25),
                                labels=scales::number_format(accuracy = 0.01))
p8b <- p8b + scale_y_continuous(limits=c(-0.55,0.5), breaks=seq(-0.5,0.5,0.25),
                                labels=scales::number_format(accuracy = 0.01))
p8b <- p8b + labs(title="", 
                  subtitle="(ii) Nestedness", 
                  x="NMDS1",y="NMDS2")
p8b <- p8b + annotate("text", x=0.4, y=0.5, label="stress = 0.184", cex=5)
p8b <- p8b + scale_colour_manual(name="Site",
                                 values=c("grey30","goldenrod2","dodgerblue3"))
...
## Plot data frame
p8c <- ggplot(site.total.NMDS, aes(x=NMDS1, y=NMDS2, colour=Site))
p8c <- p8c + geom_point(cex=2, alpha=0.3) + stat_ellipse()
p8c <- p8c + scale_x_continuous(limits=c(-0.2,1), breaks=seq(-0.2,1,0.2),
                                labels=scales::number_format(accuracy = 0.1))
p8c <- p8c + scale_y_continuous(limits=c(-0.01,0.03), breaks=seq(-0.01,0.03,0.01),
                                labels=scales::number_format(accuracy = 0.01))
p8c <- p8c + labs(title="", 
                  subtitle=expression(bold("(iii) Total"~beta~"Diversity")),
                  x="NMDS1",y="NMDS2")
p8c <- p8c + annotate("text", x=0.85, y=0.03, label="stress = 0.077", cex=5)
p8c <- p8c + scale_colour_manual(name="Site",
                                 values=c("grey30","goldenrod2","dodgerblue3"))
...
## Plot all diversity results
ggarrange(p5,
          ggarrange(p8a,p8b,p8c, nrow=3, ncol=1, align="hv"), 
          nrow=1, ncol=2,
          common.legend=TRUE, legend="right")
